...
(defn validate-link-found [input]
  (let [login-link (-> input ::login-link)]
    (when-not login-link
      (throw (err :use-login-link/login-link-not-found input)))
    input))

(defn validate-link-not-used [input]
  (let [login-link (-> input ::login-link)]
    (when (login-link/used? login-link)
      (throw (err :use-login-link/login-link-already-used input)))
    input))

(defn validate-link-not-expired [input]
  (let [login-link (-> input ::login-link)]
    (when (login-link/expired? login-link)
      (throw (err :use-login-link/login-link-expired input)))
    input))
...
(defmethod view-use-login-link-err :use-login-link/login-link-not-found [_ex _request] 
  [:div "Login link was not found"])

(defmethod view-use-login-link-err :use-login-link/login-link-already-used [_ex _request]
  [:div "Login link has already been used"])

(defmethod view-use-login-link-err :use-login-link/login-link-expired [_ex _request]
